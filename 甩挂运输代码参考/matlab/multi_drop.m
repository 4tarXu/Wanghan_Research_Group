...
rho = 0.1;                                %      - 信息素蒸发系数
Q_ant = 10;                               %      - 信息素的更新量参数(根据目标函数修改，通常为百分之10到百分之20)
...
Opt_Vei_Num = 0 ;                         %      - 最优车辆数控制策略，在此策略下，后一次迭代搜索到最优解使用的车辆数不大于前一代
%            - 因为编码受车辆数的影响，因此引入这个策略的话也能一定程度降低算法求解时间
Stop_iter = 0 ;                           %      - 迭代终止策略，如果等于1，那么如果Stop_iter_Num次仍没有搜索到新的最优解话，则算法直接终止
No_better_Num = 0;                        %      - 记录算法已经 No_better_Num 次没找到新的最优解
Stop_iter_Num = 50;                       %      - 需要特别注意算法终止时的画图问题
