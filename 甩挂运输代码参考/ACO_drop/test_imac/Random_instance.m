...
%       - 输入生成需求 - 
Cus_size = 3;                                                                %       - 客户点数量   
Task_size =3;                                                               %       - 任务点数量(必须大于客户点数量，为了方便计算，取客户点数量的1、1.5、2倍)
Task_inbound_size = 1;                                                    %       - 进口客户需求数量(先DF，后PE)
...
for i = 1:Task_size
        if Task_IO(i)==1   %   - 任务为进港任务时
            ...
            Task(2*i-1,2) = find(Task_Cus(i)==Cus_select)+3;       %  - 终止点为客户节点
            Task(2*i-1,3) = 1;                      %   标记为DF任务
            Task(2*i-1,4) = 0;                      %   无前置任务
            Task(2*i-1,5) = p;                      %   有装卸时间
            
            %   - 更新PE任务
            Task(2*i,1) = find(Task_Cus(i)==Cus_select)+3;          %   起点为客户点
           ...
           %   - 更新DE任务 
            ...
            Task(2*i-1,2) = find(Task_Cus(i)==Cus_select)+3;                        %   终点为客户点
            Task(2*i-1,3) = 3;                        %   标记为PE
            Task(2*i-1,4) = 0;                    %   前置任务为对应DF
            Task(2*i-1,5) = p;                        %   无装卸时间
