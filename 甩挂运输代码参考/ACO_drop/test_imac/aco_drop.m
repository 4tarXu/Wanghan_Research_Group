...
Cus =[40,50;40,50;40,50;30,30;92,30;38,70;0,40;60,85;38,15];
...
Task =[3,4,3,0,1;4,2,4,1,0;2,5,1,0,1;5,3,2,3,0;3,6,3,0,1;6,2,4,5,0;2,7,1,0,1;7,3,2,7,0;3,8,3,0,1;8,2,4,9,0;2,9,1,0,1;9,3,2,11,0];
...
VeiNum = 2;             %   - 车辆数量              (车辆数量目前不能设置为1，设置后出现bug，并且我懒得调)
...
for i = 1:TaskNum
    for j = 1:TaskNum    
        ...
        %-----------------------------------------------------------------------------------------------------------------------------------------------------%
          if Task(j,3) == 1                      %          - 任务 j 为 DF                                                       
                                                 %              - 衔接任务时间为 : 行驶到挂车中心时间 + 提取挂车时间 + 行驶到重箱堆场时间        
              Linktime(i,j) = Distance(Task(i,2),1)*v + t_drop + Distance(1,2)*v;     
...
%------------------------------------------------------------记   录   各   代   最   优   解--------------------------------------------------------------------------------%    
    if iter == 1                                       %   - 如果迭代次数为1，直接记录最优解和路径
        ...
        VeiNum = size(find(Best_route(:,1)),1);
        ...
        if min_obj < Best_solution(iter-1)             %   - 如果本代最优解小于当前最优解
            Best_solution(iter) = min_obj;
            Best_route = Ant_route{min_ant};
             VeiNum = size(find(Best_route(:,1)),1);
            for i = 1:TaskNum
            ...
            iter_control = iter_control +1; %   表示多少代没有找到最优解。
        end
