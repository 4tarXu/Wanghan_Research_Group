...
%Stop_iter = 0;  % 提前终止策略，算法如果在Stop_iter_Num没有找到新的最优解，那么停止迭代
%Stop_iter_Num = 100;
...
%rho = 0.1;                                %      - 信息素蒸发系数
%min_eta = 2;                               %    - 能见度eta最小值
%Q_ant = 500;                               %      - 信息素的更新量参数(根据目标函数修改，通常为百分之10到百分之20)

%Tau_min = 0.5;                          %      - 信息素浓度下界
%Tau_max = 3;                              %      - 信息素浓度上界

%iter_max = 20;                           %      - 算法最大迭代次数
...
%Cus_size = 8 ;
% - 设定任务点数量；
%Task_size = 8;    % 要求任务点数量必须要大于客户点数量
% - 设定20ft集装箱任务数量
%Task_20ft_size = 5; % 要求数值小于 Task_size
% - 计算40ft集装箱任务数量
%Task_40ft_size = Task_size - Task_20ft_size;
% - 设定20ft进港数量
%Task_20ft_inbound_size = 3; %要求数值小于 Task_20ft_size
% - 计算20ft出港数量
%Task_20ft_outbound_size = Task_20ft_size - Task_20ft_inbound_size;
% - 设定40ft进港数量
%Task_40ft_inbound_size = 2; %要求数值小于 Task_40ft_size
% - 计算40ft出港数量
%Task_40ft_outbound_size = Task_40ft_size - Task_40ft_inbound_size;


%Run_test_Num = 10;  %算法运行次数



Remainder_vol = 2*ones(VeiNum,1);       %车容设置，通过这个可以实现多车型



%   再次运行时这一行注释掉，方便算例重新运行
[Cus_select,Task_Cus,Cus,Task] = Random_instance_MICT(Cus_size,Task_size,Task_20ft_size,Task_40ft_size,Task_20ft_inbound_size,Task_20ft_outbound_size,Task_40ft_inbound_size,Task_40ft_outbound_size);

Save_path = ['Solution/',num2str(Cus_size),'-',num2str(Task_size),'(',num2str(Task_20ft_size),'(',num2str(Task_20ft_inbound_size),'-',num2str(Task_20ft_outbound_size),')-',num2str(Task_40ft_size),'(',num2str(Task_40ft_inbound_size),'-',num2str(Task_40ft_outbound_size),'))/Drop/'];

mkdir(Save_path)

%   保存实验环境到Solution

save([Save_path,'instance.mat'],'Cus_select','Task_Cus','Cus','Task','VeiNum')
...
v = 1/40;                      %   - 单位距离行驶时间
t_teu = 1/12;                %   - 单个集装箱的提箱、交箱时间(假设40尺箱和两个20尺箱都是2*t_teu的时间)
t_drop = 1/6;               %   - 甩箱时间(提取40ft是2倍，但提取2个20ft也是t_drop)

% - 车辆相关数据 -

Time_Max =36;              %   - 牵引车最长工作时间
...
for Run_test = 1:Run_test_Num
    ...
    while iter <= iter_max
        ...
        while ant <= AntNum
            ...
            for Tas = 1:TaskNum
                ...
                for i = 1:EL                                            %   - 这里 i 代表车辆编号
                    for j = 1:UL                                        %   - 这里 j 代表任务编号
                        ...
                        if End_Task(i) == 0  % - 如果这是车辆i首次调用
                            ...
                            switch Task(Unvisited_Task(j),3)   % - 如果任务j是DF_20ft
                                
                                case 1
                                
                                % - 考察离港车容量
                                if Origin_vol(i) >= 1 && Remainder_vol(i) >=1
                                    Potential_arc_linkmode(i,j) = 1;
                                else
                                ...
                                % - 考察离港车容量
                                if Origin_vol(i) >= 2 && Remainder_vol(i) >=2
                                    Potential_arc_linkmode(i,j) = 1;
                                else
                                    Potential_arc_linkmode(i,j) = 2;
                                    Additional_arc_time(i,j) = t_teu*(2-Remainder_vol(i));   % - 附加清空车上集装箱时间为。(2-Remainder_vol(i))表示车辆i上当前装载的集装箱数量
                ...
                for i = 1:EL
                    for j = 1:UL
                        ...
                        if isempty(Potential_arc{i,j})                             % - 如果弧中元素为空集，则选择概率为0
                            ...
                            Eta = min(Eta,min_eta);%如果最优解一直无法出现，适当改大改小
                ...
                %       - 若要搜索过程中不出?现不可行解，则应 : (1)延长T_max取值; (2) 增加可调用车辆数 VeiNum
                ...
                if Potential_arc_linkmode(x(s0),y(s0)) == 1
                    ...
                    switch Task(Potential_arc{x(s0),y(s0)}(2),3) 
                        case 1              % - 如果被插入的任务是DF_20ft
        ...
        if iter == 1                                       %   - 如果迭代次数为1，直接记录最优解和路径
            ...
            if min_obj < Best_solution(iter-1)             %   - 如果本代最优解小雨当前最优解
                ...
                disp(['算法已经连续：',num2str(No_better_Num),'代未找到新的最优解！'])
        ...
        Time_iter(iter) = etime(clock,iter_start);
        iter = iter + 1;                               %   - 迭代次数加 1
    ...
    figure(2)
    plot(1:plot_iter,Time_iter(1:plot_iter),'b','linewidth',1.2)
    xlabel('迭代次数')
    ylabel('迭代时间')
    ...
    % 保存图像
    filename1 = ['Solution_iter_',num2str(Run_test),'_.jpg'];
    filename2 = ['Time_iter_',num2str(Run_test),'_.jpg'];
    print(figure(1),'-djpeg',[Save_path,filename1])
    print(figure(2),'-djpeg',[Save_path,filename2])
    ...
    %   保存所有输出变量
    save([Save_path,num2str(Run_test),'.mat'],'Best_route','Best_route_mode','Best_solution','Ave_solution','Time_iter','Time_Cost','Best_iter')
