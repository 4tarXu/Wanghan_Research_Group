...
% - 设定20ft进港数量
%Task_20ft_inbound_size = 3; %要求数值小于 Task_20sft_size
% - 计算20ft出港数量
...
Save_path = ['Solution/',num2str(Cus_size),'-',num2str(Task_size),'(',num2str(Task_20ft_size),'(',num2str(Task_20ft_inbound_size),'-',num2str(Task_20ft_outbound_size),')-',num2str(Task_40ft_size),'(',num2str(Task_40ft_inbound_size),'-',num2str(Task_40ft_outbound_size),'))/Trad/'];
                        ...
                        if ismember(Task(Unvisited_Task(j),4),Visited_Task)
                            ...
                            if  Task(Unvisited_Task(j),6) <= Ori_Remainder_vol(i)
                                ...
                                if Route_time(i) + Linktime(End_Task(i)+1,Unvisited_Task(j)+1,Potential_arc_linkmode(i,j)) + Additional_arc_time(i,j)  + Tasktime(Unvisited_Task(j)+1) + Linktime(Unvisited_Task(j)+1,1,1) <= Time_Max
                                    ...
                                    if mod(End_Task(i),2) == 1         %   濡???杞?杈?????涓?涓?浠诲?℃??濂??帮?????瑕?绔??昏?块??涓?涓?涓?浠诲??
                                        if End_Task(i) == Task(Unvisited_Task(j),4)
                                            Potential_arc{i,j} = [End_Task(i),Unvisited_Task(j)];
                                        end
                                        
                                    else
                                        %濡???Unvisited_Task(j)????缃?浠诲?′???End_Task????涓?
