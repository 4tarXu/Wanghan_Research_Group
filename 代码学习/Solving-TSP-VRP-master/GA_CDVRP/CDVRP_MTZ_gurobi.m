...
load('../test_data/Travelcon.mat')	  %行程约束
    ...
    objective = 0;
    for k = 1:VehicleNum
        objective = objective + sum(sum(Distance .* x(:,:,k)));
    end
    ...
    for k = 1:VehicleNum
        ...
        for i = 1:CityNum
            for j = 1:CityNum
                if i ~= j
                    capacity_constraint = capacity_constraint + Demand(j) * x(i,j,k);
                end
            end
        end
     ...
     for k = 1:VehicleNum
         ...
         for i = 1:CityNum
             for j = 1:CityNum
                 if i ~= j
                     travel_constraint = travel_constraint + Distance(i,j) * x(i,j,k);
                 end
             end
         end
         constraints = [constraints, travel_constraint <= Travelcon];
     end



    % 9. 确保如果车辆k不访问客户i，则对应的u(i,k)不受影响
    % 通过big-M方法实现
    M = CustomerNum + 1;
    for k = 1:VehicleNum
        for i = 1:CustomerNum
