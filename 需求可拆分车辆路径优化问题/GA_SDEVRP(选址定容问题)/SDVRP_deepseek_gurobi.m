...
%% 约束条件    
  ...
  % 10.(车辆-电池)电量消耗约束组 - 修正版
M = 10000; % 大M常数

% 仓库出发时满电
for k = 1:VehicleNum
    constraints = [constraints, Yik(1,k) == B];
end
...
for k = 1:VehicleNum
    for i = ChargeStations_index
        ...
        for j = 1:CityNum
            if i ~= j
                % 如果存在从换电站i到j的弧，则换电站i的电量为B
                constraints = [constraints, Yik(i,k) >= B * Xijk(i,j,k) - M * (1 - Xijk(i,j,k))];
                constraints = [constraints, Yik(i,k) <= B * Xijk(i,j,k) + M * (1 - Xijk(i,j,k))];
            end
        end
        % 电量边界约束
        constraints = [constraints, Yik(i,k) >= 0];
        constraints = [constraints, Yik(i,k) <= B];
    end
end
...
for k = 1:VehicleNum
    for i = 1:CityNum
        for j = 1:CityNum
            if i ~= j && ~ismember(i, ChargeStations_index) && i ~= 1
                % 普通节点间的电量转移
                constraints = [constraints, Yik(j,k) <= Yik(i,k) - Distance(i,j) * Xijk(i,j,k) + B * (1 - Xijk(i,j,k))];
            end
        end
    end
end
...
for k = 1:VehicleNum
    for i = 1:CityNum
        for j = 1:CityNum
            if i ~= j
                constraints = [constraints, Yik(i,k) >= Distance(i,j) * Xijk(i,j,k) - M * (1 - Xijk(i,j,k))];
            end
        end
    end
end
...
for k = 1:VehicleNum
 ...
 %% 求解器参数   
    ...
    ops = sdpsettings('verbose', 1, 'solver', 'gurobi');
     ops.gurobi.TuneTimeLimit = 100;    % 设置5分钟调参时间

    % 其他必要设置
     ops.gurobi.TimeLimit = 100;       % 求解时间限制
