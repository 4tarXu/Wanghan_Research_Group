function [TSPRoute,Chrom,VehicleNum]=InitPop(NIND,CityNum,Demand,Capacity,Travelcon,Distance)
%% 初始化种群
%输入：
%NIND       种群大小
%CityNum	需求点个数
    ...
    for j=2:CityNum+1
        ...
        if DisTraveled + Distance(VRProute(k-1),TSProute(j)) + Distance(TSProute(j),1) > Travelcon
           ...
           if delivery+Demand(TSProute(j)) > Capacity %这一点配送完成后车辆可配送量超容量约束
            ...
            VRProute(k)=TSProute(j); %TSP路线中此点添加到VRP路线
            DisTraveled = DisTraveled + Distance(VRProute(k-1),TSProute(j));  % 初始化距离为配送中心到此点距离
              end
