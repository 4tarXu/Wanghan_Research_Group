function [Chrom_sel,VehicleNum_sel] = TSPtoChrom(TSPRoute_sel,CityNum,Distance,Travelcon,Demand,Capacity)
...
TSP_size=size(TSPRoute_sel,1);
    ...
    for i=1:TSP_size
        ...
        for j=2:CityNum+1
            k=k+1;   %对VRP路径下一点进行赋值
                    % 第一步检查里程约束
        if DisTraveled + Distance(VRProute(k-1),TSPRoute_sel(i,j)) + Distance(TSPRoute_sel(i,j),1) > Travelcon
            VRProute(k) = 1;
